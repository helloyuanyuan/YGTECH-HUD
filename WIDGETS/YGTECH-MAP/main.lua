local name = "YGTECH-MAP"

-- 常量定义
local CONST = {
    MIN_SATS_FOR_HOME = 6,
    GPS_CALC_FACTOR = 111000,
    ...
    DIST_METER_MAX = 10000,
    DIST_KM_MAX = 100000000,
    PITCH_RANGE = 35,
    PITCH_RATE = 1.8,
...
local function draw_plane4( yaw )
	local lon1 = lon
	local lon2 = home_x
	local lat1 = lat
	local lat2 = home_y
	...
	lcd.drawBitmap(home, cx- 50 * (size+1)*7/100 , cy- 43 * (size+1)*7 /100, (size+1)*7 )
	if map_dis<10000 then
		lcd.drawNumber(hx-50-len(map_dis)*4,hy-8,map_dis, SMLSIZE + lcd.RGB(120,120,120))
		lcd.drawNumber(hx+50-len(map_dis)*4,hy-8,map_dis, SMLSIZE + lcd.RGB(120,120,120))
	end	
	x_point[1],y_point[1] = point_roll(posx,posy-5*(size+1)/4,math.tointeger(yaw+180))
	x_point[2],y_point[2] = point_roll(posx,posy+12*(size+1)/4,math.tointeger(yaw))
...
local function message_refresh()
	if message_time > 0 then
		...
		XUI_bar(hx,hy,len*4+30,20,6,WHITE)
...
local function refresh(widget, event, touchState)
	...
	if event == nil and w < 400 and h < 300 then 	
		...
		lcd.drawFilledRectangle( 0,0,lx, ly, MAP_BG_COLOR )

		lcd.drawLine(0,hy,lx,hy,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(0,hy-50,lx,hy-50,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(0,hy+50,lx,hy+50,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(0,hy-100,lx,hy-100,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(0,hy+100,lx,hy+100,DOTTED,MAP_LINE_COLOR)		

		lcd.drawLine(hx,0,hx,ly,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(hx-50,0,hx-50,ly,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(hx-100,0,hx-100,ly,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(hx+50,0,hx+50,ly,DOTTED,MAP_LINE_COLOR)
		lcd.drawLine(hx+100,0,hx+100,ly,DOTTED,MAP_LINE_COLOR)		
		...
		draw_plane4(yaw)
		lcd.drawText(0, hy-8,"W", BOLD + FONT_COLOR )		
		lcd.drawText(hx-4, 0,"N", BOLD + FONT_COLOR )
		lcd.drawText(lx-12, hy-8,"E", BOLD + FONT_COLOR )
		lcd.drawText(hx-4, ly-16,"S", BOLD + FONT_COLOR )	
		
		lcd.drawText(4, ly-32,"LON", SMLSIZE + FONT_COLOR )
		lcd.drawText(4 , ly-16, tostring(lon).. "E", SMLSIZE + FONT_COLOR )
		
		lcd.drawText(lx-28, ly-32,"LAT", SMLSIZE + FONT_COLOR )	
		lcd.drawText(lx-65 , ly-16, tostring(lat) .. "N",  SMLSIZE + FONT_COLOR )
		
		lcd.drawText(4, 2,"GPS",SMLSIZE + FONT_COLOR )	
		lcd.drawText(4, 18,getValue('Sats'),SMLSIZE + FONT_COLOR )	
		
		lcd.drawText(lx-36, 2,"Hdop" , SMLSIZE + FONT_COLOR )
		lcd.drawText(lx-36, 18,tostring(getValue('Hdg')), SMLSIZE + FONT_COLOR )
				
		hud_distance=get_distance(lon,lat,home_x,home_y)*1000;
		if hud_distance < 10000 then
			lcd.drawText(hx-18-len(hud_distance)*4, ly-32,"Dist: " .. string.format("%.1f", hud_distance ) .. "m", SMLSIZE + FONT_COLOR )		
		elseif hud_distance < 100000000 then
			lcd.drawText(hx-18-len(hud_distance/1000)*4, ly-32,"Dist: " .. string.format("%d", hud_distance /1000) .. "km", SMLSIZE + FONT_COLOR )			
		else	
			lcd.drawText(hx-30, ly-32,"Dist:NoGPS",  SMLSIZE + FONT_COLOR )
		end
